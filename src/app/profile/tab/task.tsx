...
function TasksTab() {
  const [taskFilter, setTaskFilter] = useState<"all" | "daily" | "personal" | "team">("all");
  const [selectedTask, setSelectedTask] = useState<number | null>(null);
  const [viewMode, setViewMode] = useState<"list" | "grid">("grid");

  const allTasks: Task[] = [...mockTasks.daily, ...mockTasks.personal, ...mockTasks.team];
                  ...
                  <div className="space-y-2 mb-3">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-slate-300">
                        Tiến độ:{" "}
                        {task.jobs.filter((j) => j.status === "completed").length}
                ...
                <div className="space-y-6">
                  ...
                  <div className="rounded-lg border border-slate-800 bg-slate-950 p-5 sm:p-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                      ...
                      <div className="bg-slate-900 p-3 rounded-lg">
                        <div className="text-xs text-slate-500 mb-1">Điểm XP</div>
