...
function SkillsTab({ userInfo }: any) {
  ...
  const hardSkills = userInfo.kpis?.map((skill: any) => ({
    icon: skill.icon || "💼",
    name: skill.kpi.name,
  ...
  const ratingStats = hardSkills.reduce((acc: any, skill: any) => {
    const rating = getRating(skill.value);
    acc[rating as keyof typeof acc] = (acc[rating as keyof typeof acc] || 0) + 1;
    return acc;
  }, { S: 0, A: 0, B: 0, C: 0 });

  const averageScore = hardSkills.reduce((sum:any, skill: any) => sum + skill.value, 0) / hardSkills.length;
        ...
        <div className="bg-slate-800/30 rounded-lg border border-slate-700 p-4">
          ...
          <div className="mb-6 p-4 bg-slate-800/50 rounded-lg border-2 border-slate-600">
            <div className="text-center">
              <p className="text-xs text-slate-400 mb-2">Tổng quan</p>
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full border-4 ${getRatingColor(overallRating)}`}>
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-slate-400 mb-3">Phân bố đánh giá</p>
            {["S", "A", "B", "C"].map((rating) => {
              const count = ratingStats[rating as keyof typeof ratingStats];
              const percentage = (count / hardSkills.length) * 100;
              return (
                <div key={rating} className="space-y-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 text-xs font-bold rounded border ${getRatingColor(rating)}`}>
                        {rating}
                      </span>
