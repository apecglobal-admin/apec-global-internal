...
const CalendarDefault = ({
    ...
    return (
        <div
            className={`rounded-3xl bg-box-shadow bg-white p-5 sm:p-6 ${className}`}
        >
            {/* Calendar Header */}
            ...
            <div className="mt-2 grid grid-cols-7 gap-2">
                {calendarDays.map((day, index) => {
                    ...
                    const hasInternal = dayEvents.some(
                        (e: any) => e.type === "internal"
                    );
                    const hasExternal = dayEvents.some(
                        (e: any) => e.type === "external"
                    );
                    ...
                    const hasReminderActive = !isPast && dayEvents.some((e: any) => hasReminder(e.id));

                    let bgClass = "bg-gray-200 text-slate-400"; // mặc định
                    
                    // Nếu ngày có sự kiện
                    if (dayEvents.length > 0) {
                      if (isPast) {
                        // Ngày đã qua và có sự kiện → màu riêng
                        bgClass = "bg-[#97cadb] text-white border-gray-500";
                      } else if (hasInternal && hasExternal) {
                        bgClass = "bg-gradient-to-br from-blue-500 to-emerald-500 text-white border-blue-400";
                      } else if (hasInternal) {
            ...
            {showLegend && (
                <div className="mt-4 space-y-2 text-xs text-black">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="h-3 w-3 rounded-full bg-[#97cadb]"></span>
