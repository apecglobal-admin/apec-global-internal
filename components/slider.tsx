...
<div className="flex flex-col sm:flex-row items-center justify-between rounded-3xl border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900/80 to-slate-950 px-6 py-4">
  ...
  <div className="flex flex-col items-center sm:items-start text-center sm:text-left sm:w-2/3">
        ...
        {slides.map((_, idx) => (
          <div
            ...
            className={`h-1.5 w-8 rounded-full transition sm:w-12 ${
              idx === current ? "bg-blue-500" : "bg-slate-700"
            }`}
          ></div>
        ))}
      </div>
    ...
    <div className="mt-5 text-2xl font-semibold text-white sm:text-3xl md:text-4xl">
      {slides[current].title}
    </div>
    <div className="mt-3 max-w-3xl text-base text-slate-300 sm:text-lg">
    ...
    <div className="mt-6 flex flex-wrap justify-center sm:justify-start gap-3">
      <button
        onClick={prev}
        className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-700 bg-slate-900 text-slate-200 transition hover:border-blue-500"
      >
        <ChevronLeft size={22} />
      </button>
      <button
  ...
  {/* Hình ảnh bên phải */}
  <div className="flex justify-center sm:w-1/3 mt-8 sm:mt-0">
    <Image
